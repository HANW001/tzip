...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        #     if t[0][i] ==59:
        #         print()
        # a = t[3][0].torch().from_numpy()
        # a = torch.from_numpy(t[3][0])
        # print(t[0][0])

        # a = t[3][0].cpu().detach().numpy()
        # i = 0
        # np.savetxt('area.csv', a , delimiter = ',')
        # i += 1
        # print(type(a))
        #print(len(t[0]))
...
def area_setting():
    ...
    cv2.imwrite('current_shot.jpg', image)

    camera.release()
    cv2.destroyAllWindows()
    image_path  = 'current_shot.jpg'
...
def detecting():
    ...
    while True:
        ...
        point =  util.point(canvas, candidate, subset)
        print('point: ',point)
...
def main():
    while True:
        print('메뉴를 선택해주세요.\n 1. 영역 설정  2. 실시간 탐지  3. 종료')
        sel = int(input('번호 입력 후 엔터 : '))
        if sel == 1:
            print('영역 설정을 진행합니다.\n모델 가동 후 현재 카메라 화면이 캡쳐됩니다.')
            area_setting()
        elif sel == 2:
