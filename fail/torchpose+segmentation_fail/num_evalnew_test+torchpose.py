...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        # print(t[3][0][0][0])
        if t[3][5][390][785].cpu().numpy() == 1.0:
            print('위험')
        else:
            print('안전')
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def prep_frame(inp, fps_str, candidate, subset, canvas):
        with torch.no_grad():
            frame, preds = inp
            ...
            ######
            return prep_display(preds, frame, None, None, undo_transform=False, cla4ss_color=True, fps_str=fps_str),candidate, subset, canvas
            #######
    frame_buffer = Queue()
    video_fps = 0
