...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            # 밑에 if이 txt 표시
            if cfg.dataset.class_names[classes[j]] == 'cup':
    ...
    while True:
        ...
        if len(body_point) == 0:
            ...
            if sum(state) > 0:
                ...
                state_time += 1
                if state_time >= 3:
                    playsound( "C:/Users/kjh/Desktop/yolact/alarm1.mp3" )
                    state_time = 0
            else:
                print('안전한 상태입니다.')
                state_time = 0
        ...
        # pose_point = util.get_pose_point()
        
        if len(RShoulder_point) == 1 and len(LShoulder_point) == 1 and len(Nose_point) == 1 and len(Neck_point):
            ...
            if (LShoulder_x-RShoulder_x)**2 + (LShoulder_y-RShoulder_y)**2 >= ((Nose_x-Neck_x)**2 + (Nose_y-Neck_y)**2)*0.85:
                current_pose = 'F'
                print('앞으로 누워있습니다.')
            elif (LShoulder_x-RShoulder_x)**2 + (LShoulder_y-RShoulder_y)**2 < ((Nose_x-Neck_x)**2 + (Nose_y-Neck_y)**2)*0.85:
                if Nose_x > Neck_x:
                    current_pose = 'L'
                    print('왼쪽으로 누워있습니다.')
                else:
        ...
        cv2.imshow('Realtime Detecting', canvas)#一个窗口用以显示原视频
...
def video_detecting():
    ...
    while True:                 # 무한 루프
        ...
        if len(body_point) == 0:
            cv2.putText(canvas, str3, (60, 230), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255))
            ...
            for i in range(len(body_point)):
                ...
                if target_area[y][x] == 1:
                    cv2.putText(canvas, str2, (60, 230), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255))
                else:
                    cv2.putText(canvas, str1, (60, 230), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255))

        cv2.imshow('Video Detecting per frame', canvas)

        out.write(canvas) # 영상 데이터만 저장. 소리는 X

        if cv2.waitKey(delay) == 'q': # esc를 누르면 강제 종료
            break
